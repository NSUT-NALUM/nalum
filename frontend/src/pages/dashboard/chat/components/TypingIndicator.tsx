...
export const TypingIndicator = ({ conversationId }: TypingIndicatorProps) => {
  ...
  return (
    <div className="px-4 py-2">
      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-2xl bg-card border border-border shadow-sm animate-in fade-in slide-in-from-bottom-1">
        <div className="flex gap-1">
          <span className="w-1.5 h-1.5 bg-muted-foreground rounded-full animate-bounce" style={{ animationDelay: "0ms" }} />
          <span className="w-1.5 h-1.5 bg-muted-foreground rounded-full animate-bounce" style={{ animationDelay: "150ms" }} />
          <span className="w-1.5 h-1.5 bg-muted-foreground rounded-full animate-bounce" style={{ animationDelay: "300ms" }} />
        </div>
        <span className="text-[10px] text-muted-foreground font-medium">Typing...</span>
      </div>
    </div>
  );
};
